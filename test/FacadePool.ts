...
export async function makeTokenSeq(wbera: WBERA): Promise<TestPool[]> {
    let factory = await ethers.getContractFactory("MockERC20") as ContractFactory
    ...
    let poolM = await makePoolFrom(tokens[0], tokens[1], wbera)
    let poolN = await makePoolFrom(tokens[1], tokens[2],wbera, await poolM.dex)
    let poolO = await makePoolFrom(tokens[2], tokens[3],wbera, await poolM.dex)
    return [poolM, poolN, poolO]
}

export async function makeTokenTriangle(wbera: WBERA): Promise<TestPool[]> {
    let factory = await ethers.getContractFactory("MockERC20") as ContractFactory
    let tokenX = new ERC20Token((await factory.deploy() as MockERC20))
    let tokenY = new ERC20Token((await factory.deploy() as MockERC20))
    let tokenZ = new ERC20Token((await factory.deploy() as MockERC20))
    ...
    let poolM = await makePoolFrom(tokens[0], tokens[1], wbera)
    let poolN = await makePoolFrom(tokens[1], tokens[2],wbera, await poolM.dex)
    let poolO = await makePoolFrom(tokens[2], tokens[0],wbera,  await poolM.dex)
    return [poolM, poolN, poolO]
}

export async function makeTokenNext(pool: TestPool, wbera: WBERA): Promise<TestPool> {
    let factory = await ethers.getContractFactory("MockERC20") as ContractFactory
    let tokenZ = new ERC20Token((await factory.deploy() as MockERC20))

    return makePoolFrom(pool.quote, tokenZ, wbera)
}
...
export class TestPool {
    ...
    constructor(base: Token, quote: Token,wbera: WBERA, dex?: CrocSwapDex) {
        ...
        if (dex) {
            ...
            this.dex = buildCrocSwapSex(this.auth, wbera)
        }
