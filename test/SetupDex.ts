...
import { CrocSwapDex, WBERA } from "../typechain";
...
export async function buildCrocSwapSex (auth: Promise<Signer>, wbera: WBERA): Promise<CrocSwapDex> {
    const abi = new AbiCoder()

    let factory = await ethers.getContractFactory("CrocSwapDex")
    let dex = await factory.connect(await auth).deploy(wbera.address) as CrocSwapDex

    factory = await ethers.getContractFactory("ColdPath")
    let proxy = await factory.deploy(wbera.address)
    ...
    proxy = await factory.deploy(wbera.address)
