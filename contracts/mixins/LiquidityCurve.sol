...
contract LiquidityCurve is StorageLayout {
     ...
     * @dev    Use for light-weight gas ergonomics when iteratively operating on the
     *         curve. But it's the callers responsibility to persist the changes back
     *         to storage when complete. */
    function snapCurve(
        bytes32 poolIdx
    ) internal view returns (CurveMath.CurveState memory curve) {
     ...
     *         responsibility */
    function snapCurveInit(
        bytes32 poolIdx
    ) internal view returns (CurveMath.CurveState memory) {
    ...
    function snapCurveInRange(
        ...
        require(
            curve.priceRoot_ >= minPrice && curve.priceRoot_ <= maxPrice,
            "RC"
        );
    }
     ...
     *                 following the addition of this liquidity. */
    function liquidityReceivable(
        CurveMath.CurveState memory curve,
        uint128 liquidity,
        int24 lowerTick,
        int24 upperTick
    ) internal pure returns (uint128, uint128) {
        (uint128 base, uint128 quote, bool inRange) = liquidityFlows(
            curve.priceRoot_,
            liquidity,
            lowerTick,
            upperTick
     ...
     *              rate on the curve. (See CurveMath.sol. liq = seed * deflator)
    ...
    function stackRewards(
        ...
        if (rewardRate > 0) {
            ...
            // incremental accumulation (see CurveAssimilate.sol). This mathematically
            // guarantees that we never try to burn more tokens than exist on the curve.
            uint128 rewards = FixedPoint
                .mulQ48(liquidity, rewardRate)
                .toUint128By144();

            if (rewards > 0) {
                (uint128 baseRewards, uint128 quoteRewards) = liquidityPayable(
                    curve,
                    rewards
                );
                base += baseRewards;
    ...
    function liquidityPayable(
        ...
        bool inRange;
        (base, quote, inRange) = liquidityFlows(
            curve.priceRoot_,
            liquidity,
            lowerTick,
            upperTick
     ...
     *         collateral. */
    function chargeConservative(
        uint128 liqBase,
        uint128 liqQuote,
        bool inRange
    ) private pure returns (uint128, uint128) {
        return (
            (liqBase > 0 || inRange) ? liqBase + TOKEN_ROUND : 0,
